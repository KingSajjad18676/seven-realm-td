...
func handle_ground_tap(pos: Vector2, screen_x_ratio: float = 0.5) -> void:
	var target := hero
	if is_brothers_mode():
		var slot_index := 0 if screen_x_ratio < 0.5 else 1
		target = get_hero_for_slot(slot_index)
	if target == null or context == null:
		return
	if target.is_dead():
		return
	...
	if context.tower_manager:
		var tower := context.tower_manager.find_tower_at(pos)
		if tower != null:
			target.tether_to_tower(tower)
			return
		if context.tower_manager.is_valid_build_position(pos):
			return
	target.move_to(pos)
